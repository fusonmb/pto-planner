...
Run:  python pto_planner.py            (opens http://localhost:8765)
Data: PTO days, labels, and custom holidays are saved to pto_data.json
      next to this file.  Company holidays through 2030 (from the MITRE
      pay calendars) are built in.
"""
...
DATA_FILE = Path(__file__).with_name("pto_data.json")
...
def load_data() -> dict:
    """Return {'entries': {date: {hours, label}}, 'holidays': {date: name},
    'removedHolidays': [date]}."""
    data = {"entries": {}, "holidays": {}, "removedHolidays": [],
            "anchor": None, "hireDate": None}
    if DATA_FILE.exists():
        try:
            ...
            for k, v in raw.get("entries", {}).items():
                if isinstance(v, dict):
                    data["entries"][k] = {"hours": float(v.get("hours", 0)),
                                          "label": str(v.get("label", ""))}
                else:                       # old format: bare number
                    data["entries"][k] = {"hours": float(v), "label": ""}
...
def compute_projection(entries: dict, today: date,
                       ...
                       hire_date: date = DEFAULT_HIRE_DATE) -> list:
    """Balance at every period Sunday from the anchor to ~36 months out.

    Each period Sunday S covers the 14 days (S-13 .. S).  Usage in the window
    is subtracted, then the accrual is added, then the cap is applied.  From
    the nine-year employment mark the accrual and cap step up.  The balance
    may go negative; negative periods are flagged.  The running balance keeps
    full precision; rows report it rounded for display.
    """
    ...
    for dstr, e in entries.items():
        try:
            parsed[date.fromisoformat(dstr)] = float(e.get("hours", 0))
    ...
    while sunday <= end:
        ...
        used = round(sum(h for d, h in parsed.items()
...
def state_payload() -> dict:
    ...
    return {
        ...
        "projection": compute_projection(data["entries"], today,
                                         anchor_date, anchor_balance, hire),
    }
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/" or path.startswith("/index"):
            ...
            self._send(200, html.encode(), "text/html; charset=utf-8",
                       {"Content-Disposition":
                        'attachment; filename="pto_plan_snapshot.html"'})
        elif path == "/api/state":
    ...
    def _post_entry(self):
        try:
            ...
            return
        if not (0 <= hours <= 24):
            self._json({"ok": False, "error": "Hours must be 0-24."}, 400)
            return
        with _lock:
            data = load_data()
            ...
            hol_skipped = []
            if hours > 0 or label:
                # PTO (and labels) never land on weekends or holidays
            ...
            for day in valid:
                key = day.isoformat()
                if hours <= 0 and not label:
                    data["entries"].pop(key, None)
                else:
                    data["entries"][key] = {"hours": round(hours, 2),
                                            "label": label}
            save_data(data)
...
<title>PTO Balance Planner</title>
<script>
...
<div class="wrap">
  ...
  <p class="sub">6.7692 h accrue every two weeks, 8.000 h after nine years
     (balance posts every other Sunday) &middot;
     cap 240 h, 320 h after nine years &middot; <span id="subAnchor"></span> &middot;
     company holidays through 2030 built in (weekends &amp; holidays never receive PTO).
     Calendar shading alternates by pay period.
     Click a day &mdash; or click and drag &mdash; to mark PTO.
  ...
  <div class="cols">
    <div class="card">
      ...
      <div class="editor" id="editor" style="display:none">
        ...
        <div class="row">
          <input type="number" id="edHours" min="0" max="24" step="0.25"> h/day
          <input type="text" id="edLabel" maxlength="80"
                 placeholder="Label (e.g. Hawaii trip)">
        </div>
        <div class="row">
          <button id="edFull">Full day (8h)</button>
          <button id="edHalf">Half (4h)</button>
        ...
        <div class="hint">0&ndash;24 hours per day; use 0 or Clear to remove.
          Labels are saved with the days. Weekends and holidays are always
          skipped &mdash; they never receive PTO.</div>
    ...
    <div class="card">
      ...
      <div class="legend-note">Balance at each period Sunday. Dashed line = PTO cap
        (steps up at the 9-year mark); red points = period overdrawn.
...
function renderTiles() {
  ...
  const futureDays = Object.entries(STATE.entries).filter(([d, e]) => d > today && e.hours > 0);
  const plannedFuture = futureDays.reduce((a, [, e]) => a + e.hours, 0);
...
function renderCalendar() {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    const e = STATE.entries[ds];
    if (e && e.hours > 0) inner += `<span class="pto">${fmtU(e.hours)}${unitSuffix()}</span>`;
...
function openEditor() {
  ...
  const existingH = new Set(dates.map(d => entryHours(d)));
  const uniform = existingH.size === 1 ? [...existingH][0] : 0;
...
async function post(url, body) {
  if (IS_STATIC) {
    $("edErr").textContent = "This is a static snapshot — run pto_planner.py to edit.";
    return null;
  }
...
async function saveHours(hours) {
  ...
  if (!dates.length) {
    $("edErr").textContent =
        "Weekends and holidays never receive PTO — nothing to save.";
    return;
  }
  const out = await post("/api/entry", {dates, hours, label});
  if (!out) return;
...
function periodLabels(row) {
  ...
  for (const [d, e] of Object.entries(STATE.entries)) {
    if (d >= s0 && d <= s1 && e.hours > 0 && e.label)
      groups[e.label] = (groups[e.label] || 0) + e.hours;
  }
...
function renderChart() {
  ...
  s += `<polyline fill="none" stroke="var(--series)" stroke-width="2"
  ...
  rows.forEach((r, i) => {
    const [px, py] = pts[i];
    const c = r.overdrawn ? "var(--critical)" : "var(--series)";
  ...
  s += `<text class="dlabel" x="${pts[pts.length - 1][0] + 6}"
  ...
  for (const el of svg.querySelectorAll(".hover-target")) {
    el.addEventListener("mouseenter", () => {
      ...
      if (r.used)    html += `<div class="t-line">&minus;${fmtU(r.used)} ${unitSuffix()} used</div>`;
      ...
      tip.style.top = Math.max(pts[+el.dataset.i][1] - tip.offsetHeight - 12, 0) + "px";
    });
...
function renderHire() {
  ...
  $("hireInfo").textContent = "9-year mark " + fmtDate(STATE.nineYearDate) +
...
$("edSave").addEventListener("click", () => saveHours(parseFloat($("edHours").value) || 0));
$("edClear").addEventListener("click", () => { $("edLabel").value = ""; saveHours(0); });
$("edFull").addEventListener("click", () => { $("edHours").value = 8; saveHours(8); });
$("edHalf").addEventListener("click", () => { $("edHours").value = 4; saveHours(4); });
$("edHours").addEventListener("keydown", e => {
  if (e.key === "Enter") saveHours(parseFloat($("edHours").value) || 0);
});
...
def main():
    ap = argparse.ArgumentParser(description="PTO balance planner")
    ...
    print(f"PTO planner running at {url}  (Ctrl+C to stop)")
